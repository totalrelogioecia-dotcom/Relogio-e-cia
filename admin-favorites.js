(() => {
  ...
  function run(){loadFavorites();const oldHub=document.getElementById('admin-hub');if(oldHub)oldHub.remove();const oldCrumb=document.getElementById('admin-module-breadcrumb');if(oldCrumb)oldCrumb.remove();const dashboard=document.getElementById('dashboard');if(dashboard)new MutationObserver(()=>{if(getComputedStyle(dashboard).display!=='none')loadFavorites()}).observe(dashboard,{attributes:true,attributeFilter:['style']})}
