...
(function () {
  ...
  function mostrarTelaRecuperacao() {
    ...
    const emailAtual = tab.querySelector('#login-email')?.value?.trim() || '';
    ...
    tab.innerHTML = `
      <div class="password-recovery" style="margin-top:8px;">
        ...
        <p class="form-note">Informe o e-mail cadastrado e enviaremos um link seguro para criar uma nova senha.</p>
        ...
        <button class="btn btn-primary" type="button" id="btn-solicitar-recuperacao" style="width:100%;justify-content:center;">Enviar link de recuperação</button>
    ...
    document.getElementById('btn-solicitar-recuperacao')?.addEventListener('click', async (event) => {
      const botao = event.currentTarget;
      const bloco = botao.closest('.password-recovery');
      const campoEmail = bloco?.querySelector('input[type="email"]');
      const aviso = bloco?.querySelector('#erro-recuperacao');
      ...
      try {
        ...
        aviso.textContent = dados.message || 'Se existir uma conta com esse e-mail, enviaremos um link para redefinir a senha.';
        aviso.style.display = 'block';
