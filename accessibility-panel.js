(() => {
  ...
  function styles(){
    ...
    const s=document.createElement('style');s.id='reloja-accessibility-panel-style';s.textContent=`
      ...
      .reloja-a11y-trigger{position:static!important;order:98;flex:0 0 44px;width:44px;height:44px;min-width:44px;min-height:44px;margin-left:10px;padding:0;border:1px solid var(--ink,#111);border-radius:0;background:var(--bg,#fff);color:var(--ink,#111);box-shadow:none;display:inline-grid;place-items:center;font:700 .82rem/1 var(--font-mono,monospace);cursor:pointer;transition:background .16s,color .16s}
      .reloja-a11y-trigger:hover,.reloja-a11y-trigger[aria-expanded=true]{background:var(--ink,#111);color:var(--bg,#fff)}.reloja-a11y-trigger:focus-visible{outline:3px solid var(--red,#b00020);outline-offset:3px}
      .site-header .nav>.nav-cta{order:97}.site-header .nav>.reloja-a11y-trigger{order:98}.site-header .nav>.nav-toggle{order:99}
      ...
      @media(max-width:640px){.site-header .nav>.nav-cta{display:none}.reloja-a11y-trigger{width:40px;height:40px;min-width:40px;min-height:40px;margin-left:auto}.reloja-a11y-backdrop{padding:72px 0 0}.reloja-a11y-panel{width:100%;max-height:calc(100vh - 72px);border-left:0;border-right:0;border-bottom:0;padding:18px}}
  ...
  function installTrigger(){
    ...
    if(!nav)return null;
    nav.querySelector('.reloja-a11y-trigger')?.remove();
    ...
    b.innerHTML='<svg class="reloja-a11y-symbol" viewBox="0 0 32 32" aria-hidden="true" focusable="false"><g fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M7.7 8.2A11.5 11.5 0 0 1 24.3 8.2"></path><path d="M6.2 13.5A11.7 11.7 0 0 0 9 23.3"></path><path d="M26 13.5A11.7 11.7 0 0 1 23 23.3"></path><path d="M11.2 26.3a11.6 11.6 0 0 0 9.6 0"></path><path d="M7.3 12.1Q16 16.3 24.7 12.1"></path><path d="M16 15.3 10.5 25"></path><path d="M16 15.3 21.5 25"></path></g><circle cx="16" cy="7.1" r="3.1" fill="currentColor"></circle><circle cx="6.2" cy="11.6" r="2.15" fill="currentColor"></circle><circle cx="25.8" cy="11.6" r="2.15" fill="currentColor"></circle><circle cx="10.2" cy="25.3" r="2.15" fill="currentColor"></circle><circle cx="21.8" cy="25.3" r="2.15" fill="currentColor"></circle></svg><span class="reloja-visually-hidden">Acessibilidade</span>';
    const catalog=nav.querySelector('.nav-cta');
    if(catalog){
      let cluster=catalog.closest('.nav-actions-cluster');
      if(!cluster){
