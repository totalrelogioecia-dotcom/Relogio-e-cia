(() => {
  ...
  function migrate(){ if(read(KEYS.scale)==null&&read(LEGACY_LARGE_TEXT_KEY)==='1')write(KEYS.scale,'112.5'); remove(LEGACY_LARGE_TEXT_KEY); root.classList.remove('reloja-large-text'); }
  function scale(){return Math.max(100,Math.min(140,Number(read(KEYS.scale)||100)))}
  ...
  function installTrigger(){const nav=document.querySelector('.site-header .nav');if(!nav)return null;const old=nav.querySelector('.reloja-a11y-trigger');if(old)old.remove();const b=document.createElement('button');b.type='button';b.className='reloja-a11y-trigger';b.setAttribute('aria-label','Abrir painel de acessibilidade');b.setAttribute('aria-haspopup','dialog');b.setAttribute('aria-controls','reloja-a11y-panel');b.setAttribute('aria-expanded','false');b.title='Acessibilidade (Alt+A)';b.textContent='A';const catalog=nav.querySelector('.nav-cta');if(catalog)catalog.insertAdjacentElement('afterend',b);else nav.appendChild(b);return b}
