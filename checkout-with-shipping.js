...
function preferencePayer(payer) {
  ...
  const zip = digits(payer?.endereco?.zip_code).slice(0, 8);
  const street = String(payer?.endereco?.street_name || '').trim().slice(0, 120);
  const match = String(payer?.endereco?.street_number || '').match(/\d+/);
  if (zip && street && match) result.address = { zip_code: zip, street_name: street, street_number: Number(match[0]) };
  if (payer?.date_created) {
    const d = new Date(payer.date_created);
    if (!Number.isNaN(d.getTime())) result.date_created = d.toISOString();
  }
  return result;
}
function paymentPayer(payer) {
  ...
  if (zip && street && streetNumber) result.address = {
    zip_code: zip,
    street_name: street,
...
function preferenceItems(items) {
  return items.map(item => {
    const result = {
      id: String(item.sku || item.id), title: item.nome.slice(0, 256), description: itemDescription(item),
      quantity: item.quantidade, currency_id: 'BRL', unit_price: Number(item.unit_price.toFixed(2)), type: 'physical'
    };
...
function pixItems(items) {
  return items.map(item => {
    const result = {
      id: String(item.sku || item.id), title: item.nome.slice(0, 256), description: itemDescription(item),
      quantity: item.quantidade, unit_price: Number((item.unit_price * 0.95).toFixed(2))
    };
    if (item.foto) result.picture_url = item.foto.slice(0, 1000);
    if (item.categoria_id) result.category_id = item.categoria_id;
    return result;
...
function receiverAddress(payer) {
  ...
  const zip = digits(e.zip_code).slice(0, 8);
  if (zip.length !== 8) return undefined;
  return {
    zip_code: zip,
    street_name: String(e.street_name || '').trim().slice(0, 120),
    city_name: String(e.city_name || '').trim().slice(0, 120),
    state_name: String(e.state_code || e.state_name || '').trim().slice(0, 120),
    street_number: Number(String(e.street_number || '').match(/\d+/)?.[0] || 0),
    country_name: 'Brasil'
  };
}
...
function registerCheckoutWithShipping(app) {
  ...
  app.post('/api/checkout', async (req, res, next) => {
    ...
    try {
      ...
      const shippingCost = Number(shipping.price.toFixed(2));
      ...
      if (metodo === 'pix') {
        ...
        const data = await payment.create({
          body: {
            ...
            additional_info: { items: pixItems(normalized) }
          },
