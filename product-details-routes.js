...
const {normalizeAvailability,preparationDays,SOB_ENCOMENDA}=require('./product-availability-service');
...
function normalize(body){
 const disponibilidade=normalizeAvailability(body.disponibilidade);
 return {
  movimento:clean(body.movimento),caixa_material:clean(body.caixa_material),pulseira_material:clean(body.pulseira_material),cor:clean(body.cor),
  diametro:clean(body.diametro),resistencia_agua:clean(body.resistencia_agua),vidro:clean(body.vidro),garantia:clean(body.garantia),conteudo_embalagem:clean(body.conteudo_embalagem),
  disponibilidade,
  prazo_preparacao_dias_uteis:disponibilidade===SOB_ENCOMENDA?preparationDays(body.prazo_preparacao_dias_uteis):15,
